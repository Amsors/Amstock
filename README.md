...
- `/lookup?q=电阻` 提供可收藏、可写入二维码的只读模糊查询页；查询范围、已删除筛选与首页共用，也接受 `code` / `serial` 查询参数以及 `/lookup/查询词` 路径形式。
- 只读查询结果可通过路径按钮，在弹窗中查看从最外层父容器到当前物品或容器的完整包含链条；接口仍需有效登录会话。
- `/display/A-12-34-567890` 提供扫码直达的独立物资展示页，显示图片、全部字段和完整父容器链条；该入口不出现在站内导航中，并适配大屏与手机。
...
- **收纳树**：查看容器与物品的嵌套结构。
- **只读查询**：通过 URL 中的名称、描述或编号模糊检索条目，并查看向上追溯的容器路径。
...
├── client/
│   └── amstock_scanner.py   # USB 扫码枪大屏展示客户端
└── README.md
```
