...
- 顶部扫码按钮调用 Google Play 服务提供的系统扫码界面；识别到合法 Amstock 编号后直接打开对应的 `/display/编号` 展示页。
...
扫码使用 Google Code Scanner，App 本身不申请相机权限。扫描模块由 Google Play 服务提供；首次使用时系统可能需要下载该模块，没有 Google Play 服务的设备无法使用此入口。
